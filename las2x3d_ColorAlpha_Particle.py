...
def dumpColor (daFile, b):
	...
	with file('ColorOut.pts', 'w') as outfile:
		for data_slice in colour:
			if counter == base:
				scaled_slice = np.divide(data_slice, 256.0)
...
def dumpColorAlpha (daFile, b):
	colour = np.vstack((daFile.red, daFile.green, daFile.blue, 1.0)).transpose()  
	counter = 1
	with file('ColorOut.pts', 'w') as outfile:
		for data_slice in colour:
			if counter == base:
...
#os.system("cat X3D_Frag1.txt CoordsOut.pts X3D_Frag_Color.txt ColorOut.pts X3D_End.txt > "+arg3 )

os.system("cat X3D_Frag1_Particle.txt CoordsOut.pts X3D_Frag_Color_Particle.txt ColorOut.pts X3D_End.txt > "+arg3 )
